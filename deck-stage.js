...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    _onKey(e) {
      ...
      // If focus is inside an expandable region (summary, details, or an
      // element flagged [data-expandable]), let Space/Enter pass through to
      // the native <details>/<summary> toggle behavior. Arrow keys still
      // drive deck navigation.
      if (t && typeof t.closest === 'function') {
        const inExpandable = t.closest('summary, details, [data-expandable]');
        if (inExpandable && (key === ' ' || key === 'Spacebar' || key === 'Enter')) {
          return;
        }
      }
